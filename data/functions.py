...
def experiment(start_time, end_time, filepath)
    ...
    experiments['endUnix'] = pd.to_tpython imedelta(experiments['endUnix'], unit='s').dt.total_seconds().astype(int)#.astype(str)
...
#Query from influxdb
def from_influx(url, token, org, measurement, start_time, end_time)
    ...
    df = data_frame[['_time', 'node']].sort_values(by=["_time"]).reset_index(drop=True)
    df["_time"] = pd.to_datetime(df["_time"])

    return df

def group_time(df, expTime, parameter)
    ...
    by_time = dfNoIndex.groupby([dfNoIndex['experiment'],dfNoIndex[parameter],pd.Grouper(key="min", freq='10s')])['messageID'].count().reset_index()
    dfAggTime = by_time.rename(columns={"messageID": "count"})
...
def generate_graph(measurement, measurement_name, topology, intv, parameter, parameter_name)
    ...
    fig.savefig('./figures/'+measurement+'_'+tp+'_'+metric+'.pdf', format='pdf', facecolor='white', edgecolor='none', bbox_inches='tight', dpi=600)
    fig.savefig('./figures/'+measurement+'_'+tp+'_'+metric+'.png', format='png', facecolor='white', edgecolor='none', bbox_inches='tight', dpi=600)
